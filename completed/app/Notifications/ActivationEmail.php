...
class ActivationEmail implements Job
{
    /**
     * @var User
     */
    ...
    {
        $url = $this->request->root() . '/activate?token=' . $this->user->resetToken();

        $out  = '<p>Please click the link below in order to activate your account</p>';
        $out .= '<a href="';
        $out .= $url;
        $out .= '" target="_blank">';
        $out .= $url;
